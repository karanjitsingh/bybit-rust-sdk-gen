...
1. Build the typescript transpiler with `npm run build`. Then run `npm run gen` to generate the Rust SDK into `./bybit-rust-sdk` and run Rust post-processing (`cargo fmt` + `cargo clippy --fix`) via `npm run gen:rust`. Note that the transpiler only converts interfaces and other types and not actual execution code, some bits of the sdk are still written by hand:
   - `bybit-rust-sdk/`
      - `Cargo.toml`
